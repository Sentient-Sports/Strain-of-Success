...
class GraphVisualisation():
    ...
    def node_label(self, node):
        #state = list(node.state[:5])
        num_injuries = sum(node.state[3].values())
        #state[2] = round(state[2],3)
        #state = tuple([state[x] for x in [1]])
        return "V%s =\n%0.2f\\nN = %d" % (num_injuries, node.get_value(), node.get_visits())
    ...
    def environment_node(self, g, node, action, level):
        ...
        for (child, probability) in node.children[action]:
            ...
            g.edge(node_id, str(child.id), str(round(probability,3)))
    ...
    def node_to_string(self, game, node, print_state, print_value):
        result = ""
        if print_state:
            result += game.to_string(node.state[1]) + "\\n"
