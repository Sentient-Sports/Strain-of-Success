...
age_players=[]
for i in range(1,20):
    player_df = pd.read_csv('MDP_Team/Feature_DF/'+str(team_id)+'/'+str(i)+'.csv')
...
player_df = pd.read_csv('MDP_Team/Feature_DF/'+str(team_id)+'/'+str(player_df_dict[team_id])+'.csv')
...
def prepare_static_features(player_feature_df,games_df, opp_mean_feature_df):
    ...
    fixtures = pd.read_csv('team_data/'+str(player_team)+'_fixtures.csv')[['game_date','Competition','season']]
